...
public class PensionSecurity extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.httpBasic().and().authorizeRequests()
                .antMatchers(HttpMethod.POST, "/createPension").hasAnyRole("ADMIN")
                .antMatchers(HttpMethod.POST, "/editPension").hasAnyRole("ADMIN")
                .antMatchers(HttpMethod.GET, "/get/allPension").hasAnyRole("ADMIN")
                ...
                .antMatchers(HttpMethod.GET, "/applicantStatus/{id}").hasAnyRole("USER")
                .antMatchers(HttpMethod.GET, "/checkBalance/{id}").hasAnyRole("USER")
                .antMatchers(HttpMethod.GET, "/checkApplication/{id}").hasAnyRole("USER")
                .antMatchers(HttpMethod.GET, "/deletePension/{id}").hasAnyRole("ADMIN").and().csrf().disable().headers()
